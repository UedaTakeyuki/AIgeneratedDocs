...
社会全体のデータ収集・管理率を $R$（Recall）、システムの純度を $P$（Precision）とする。アルゴリズム社会におけるノイズ発生係数を $\alpha$、コスト換算係数を $\beta$ と定義したとき、システムのダイナミクスは以下の数式でモデル化される。
...
xychart-beta
    ...
    y-axis "Precision / Cost Scale" 0 --> 100
    line [100, 61, 38, 24, 13, 7]
    line [1, 2.5, 6.6, 15, 40, 190]
    
ディオクレティアヌス期のローマ、ロベスピエールの恐怖政治、テイラー主義の監視、そして現代のビッグテックのタイムラインで起きているのは、すべてこの数理モデルが描く「右端の垂直上昇（コストの爆発）」そのものなのである。
