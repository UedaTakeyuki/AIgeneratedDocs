...
人間ベースの監視は財政的に自壊し、AIベースの監視は機能的に自壊する。母集団のオッズが極端に偏っているリアルな現実世界において、私たちは「見逃し（過失）」か「冤罪（過剰BAN）」のどちらかの地獄を選択するしかないのだろうか。
...
本稿がこれまで描いてきた医療崩壊、コンプライアンス過剰、ビッグテックの冤罪爆発、精度が低くエラーを起こしがちな駅の改札。これらすべての自壊ループを引き起こしている真の元凶は、Recall（網羅性）の追求そのものというよりも、**「たった1回の判定（単一試行）で即座に白黒をつけ、システムを強制閉鎖する」という設計思想の硬直性**にある。

現代社会の設計において、アルゴリズムの「1回あたりの精度」の向上に天文学的なリソースが投じられている一方で、その前提となるアーキテクチャの設計思想が、奇妙なほど「一発退場の硬直性」に囚われているように見えてならない。しかし、**「判定の試行回数（$n$）を重ねる」という手続적アプローチ**が持つ驚異的な数理の威力には、驚くほど光が当てられていないのが現状である。

#### 7.1. 【ベイズ更新の奇跡】静的なオッズを、動的な手続きでハックせよ
第5章で見た【オッズの罠】の本質とは、ターゲットが極端に少ないという「社会構造の歪み（静的なベースレート）」であった。ならば、個々のシステムの識別精度を限界突破させるために泥沼の技術競争を繰り返すのではなく、システムが扱う「母集団そのもの」を手続きによって動的に書き換えていけばいいのではないか。
...
1回目の網に引っかかった「精度16%の容疑者集団（590人）」は、裏を返せば、すでに「無実の人間が大幅に間引かれた、濃縮された母集団」へと手続的に変革されている。この集団に対し、全く別の角度から2回目の網（追試）をかける。すると、2回目のスタート地点における事前確率は、当初の1%から「16%」へと劇的に跳ね上がっている。

試行回数（$n$）を重ねるという手続的アプローチの本質とは、単にチェックを厳格化して現場にさらなる負荷をかけることではない。**「ベイズ更新」という数理の力学を用いて、社会の絶望的な【オッズの罠】を、回数を重ねるごとにパタパタと書き換え、純度を極限まで跳ね上げていくプロセス**なのである。
...
この数理グラフ and 決定境界の式が証明する通り、本稿で提示されたすべての社会崩壊リスクは、対象のドメインが持つ「想定される事前確率（陽性率）」から逆算した「複数回試行」の思想をアーキテクチャに組み込むだけで、システム自体の性能（1回あたりの精度）はそのままでパタパタと解決する。
...
* **【ビッグテックの過剰BAN】**
  ...
  * **数理的解決**：AIの1回の判定で即座にアカウントを凍結する（$n=1$）から、アイロニーを理解できないAIによる冤罪が爆バツする。7.3項の計算通り、異なる文脈で4回連続（$n=4$）してフラグが立った場合のみペナルティを課すルール（4ストライク制）を徹底すれば、単発の誤判定によるアカウント消滅は数理的に完全に防ぎ、プラットフォームの機能的自壊を回避できる。
