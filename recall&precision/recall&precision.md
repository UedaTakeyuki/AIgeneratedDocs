...
第5章で見た【オッズの罠】の本質とは、ターゲットが極端に少ないという「社会構造の歪み（静的なベースレート）」であった。ならば、個々のシステムの識別精度を限界突破させるために泥沼の技術競争を繰り返すのではない。システムが扱う「母集団そのもの（オッズ）」を手続きによって動的に書き換えていけばいいのではないか。
...
この驚異的な数理の威力を、複数の積集合（AND条件）を課した数理モデルで検証しよう。
...
このように、対象の社会現象が持つドメイン知識（$\theta$ というオッズのレアリティ）さえ分かれば、私たちはシステムを崩壊させないための手続回数を、完璧にコントロールすることが可能なのである。
