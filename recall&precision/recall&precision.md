...
本稿は、現代のアルゴリズム社会および組織統治において盲目的に追求されている「網羅主義（Recallの最大化）」が、システムの適合率（Precision / プレシージョン）を反比例の力学で破壊し、社会的なノイズ処理コストを指数関数的に暴走させる自壊メカニズムを数理モデルおよび実証ファクトを用いて解明する、しかしそれらは少数の追試をおこなえばベイズ更新の効果によって救済されるものであり、その追加コスト「遊び（Slack / 猶予）」として受け入れるだけで自滅から救われる事を提言する
...
* **ベイズ更新による救済**：数理的に救いのないように見える構造的問題を救うベイズ更新を説明し、効果の見落とされがちな追試によるベイズ更新の効果で問題が救済される事を述べる。
---
